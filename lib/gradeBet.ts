...
function gradeLeg(label: string, home: string, away: string, hg: number, ag: number): 'won' | 'lost' | 'push' | null {
  const total = hg + ag;
  switch (label) {
    ...
    // Línea entera de 2 goles: si caen exactamente 2 → devuelven el dinero (push)
    case 'Más de 2 goles': return total > 2 ? 'won' : total === 2 ? 'push' : 'lost';
    case 'Menos de 2 goles': return total < 2 ? 'won' : total === 2 ? 'push' : 'lost';
...
export function gradeBet(pickLabel: string, home: string, away: string, homeGoals: number, awayGoals: number): 'won' | 'lost' | 'push' | null {
  ...
  // En combinada: si alguna pata es push y el resto ganó → push; si alguna perdió → perdida
