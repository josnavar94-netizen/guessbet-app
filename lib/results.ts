...
export type PastResult = { kickoffAt: string; h: string; a: string; gh: number; ga: number; ph: number | null; pa: number | null };
...
export async function computePastResults(competitionCode = 'WC'): Promise<PastResult[]> {
  const { rows } = await sql`
    SELECT match_date::text, kickoff_at, home_team, away_team, home_goals, away_goals, penalty_home, penalty_away
    FROM matches
  ...
  return rows
    ...
    .map(r => ({
      ...
      gh: r.home_goals,
      ga: r.away_goals,
      ph: r.penalty_home ?? null,
      pa: r.penalty_away ?? null,
    }));
}
